...
row1 = 0
df_po = pd.DataFrame(columns=['Interface', 'Description', "Type", "VLANs/IP", "Status", "VRF", "ACL"])
for p_obj in parse.find_objects('^interface port-channel')[1:]:
    desc, typ, vlanip, status, vrf, ipacl = np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
    row1 += 1
    for c_obj in p_obj.children:
        if "description" in str(c_obj):
            desc = str(c_obj)[str(c_obj).find("description") + 12:str(c_obj).find("' (parent")]
...
#-----------------------------------------------
        if "ip access-group" in c_obj.text:
            ipacl = c_obj.text[18:].split(" ")[0] + " (" + c_obj.text[18:].split(" ")[1] + ")"
#-----------------------------------------------   
    df_po.loc[row1] = [str(p_obj)[str(p_obj).find("interface") + 10:-2], desc, typ, vlanip, status, vrf, ipacl]
