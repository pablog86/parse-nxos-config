...
def conf_excel (path):
        ...
        if p_obj.text[:18] != "vlan configuration":
            ...
            if len(p_obj.text[5:]) > 4:
                ...
                for c_obj in p_obj.children:
                    df_vlan.loc[row1] = [int(p_obj.text[5:]), str(c_obj)[str(c_obj).find("name") + 5:str(c_obj).find("' (parent")]]
    
    # VLANs Interfaces information page
    row1 = 0
    df_svi = pd.DataFrame(columns=['SVI', 'Description', "IP", "VIP", "VRF", "ACL"])
    for p_obj in parse.find_objects('^interface Vlan')[1:]:
        ...
        for c_obj in p_obj.children:
            if "description" in str(c_obj):
                desc = str(c_obj)[str(c_obj).find("description") + 12:str(c_obj).find("' (parent")]
            if "ip address" in str(c_obj):
                ipa = str(c_obj)[str(c_obj).find("ip address") + 11:str(c_obj).find("' (parent")]
            ...
            if "vrf member" in str(c_obj):
                vrf = str(c_obj)[str(c_obj).find("vrf member") + 11:str(c_obj).find("' (parent")]  
    #-----------------------------------------------
            if "ip access-group" in c_obj.text:
                ipacl = c_obj.text[18:].split(" ")[0] + " (" + c_obj.text[18:].split(" ")[1] + ")"
    #-----------------------------------------------    
        df_svi.loc[row1] = [int(str(p_obj)[str(p_obj).find("interface Vlan") + 14:-2]), desc, ipa, ipv, vrf, ipacl]
    
    # Interfaces information page
    row1 = 0
    df_int = pd.DataFrame(columns=['Interface', 'Description', "Type", "VLANs/IP", "Po", "Status", "VRF", "ACL"])
        ...
        for c_obj in p_obj.children:
            ...
            try:
                if "access vlan" in str(c_obj):
                    typ = "switchport"
                    vlanip = str(c_obj)[str(c_obj).find("access vlan") + 12:str(c_obj).find("' (parent")]
                if "trunk allowed vlan" in str(c_obj):
                    if "add" in str(c_obj):
                        ...
                        typ = "switchport"
                        vlanip = str(c_obj)[str(c_obj).find("trunk allowed vlan") + 19:str(c_obj).find("' (parent")]
                if "ip address" in str(c_obj):
                    vlanip = str(c_obj)[str(c_obj).find("ip address") + 11:str(c_obj).find("' (parent")]
            except Exception as ex:
                print("Problem with line ", str(p_obj), ex)
                #TODO: add something to the table
            if "channel-group" in str(c_obj):
                po = str(c_obj)[str(c_obj).find("channel-group") + 14:str(c_obj).find("' (parent")]
    ...
    #-----------------------------------------------
            if "ip access-group" in c_obj.text:
                ipacl = c_obj.text[18:].split(" ")[0] + " (" + c_obj.text[18:].split(" ")[1] + ")"
    #-----------------------------------------------
        df_int.loc[row1] = [str(p_obj)[str(p_obj).find("interface") + 10:-2], desc, typ, vlanip, po, status, vrf, ipacl]
    
    # Port-channels information page
    row1 = 0
    df_po = pd.DataFrame(columns=['Interface', 'Description', "Type", "VLANs/IP", "Status", "VRF", "ACL", "VPC"])
    ...
    #-----------------------------------------------
            if "ip access-group" in c_obj.text:
                ipacl = c_obj.text[18:].split(" ")[0] + " (" + c_obj.text[18:].split(" ")[1] + ")"
    #-----------------------------------------------   
        df_po.loc[row1] = [str(p_obj)[str(p_obj).find("interface") + 10:-2], desc, typ, vlanip, status, vrf, ipacl, vpc]
    
    #Static Routing
    row1 = 0
    df_route = pd.DataFrame(columns=['Network', 'Next-hop', "pref", "name", "tag", "track", "VRF"])
    ...
    #IP ACLs
    row1 = 0
    df_ipacl = pd.DataFrame(columns=['Name', "rule", "action", "protocol", "src", "src_oper", "src_port", "dst", "dst_oper", "dst_port", "flags", "statistics"])
...
if __name__ == "__main__":
    ...
    if args.folder != True:
        ...
        conf_excel (os.path.join(os.path.dirname(path), wbname))
    else:
        ...
        for i in onlyfiles:
            ...
            try:
                parse = CiscoConfParse(os.path.join(path, i), syntax='nxos')
            except UnicodeDecodeError as uni:
                print ("USE A TEXT FILE!!! Closing...")
                exit()
            conf_excel (os.path.join(path2, wbname))
    print("")
